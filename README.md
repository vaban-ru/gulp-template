...
* Папка ```src``` - используется во время разработки:
    ...
    * ```svg``` — папка для SVG файлов, дял последующей генерации SVG спрайта которые автоматически сгенерируется в папке img
...
Команды для сборки:
 ...
 - `yarn build | npm run build` запускает сборку и на выходе получаем собранные, но не минифицированные  файлы app.js и app.css для последующей передачи программистам
